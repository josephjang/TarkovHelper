...
- **Change Proposal** (`decisions/`): 한 변경의 의도와 판단 기록. 형식과 규칙은
  `decisions/README.md` 참고.
...
단일 변경에 딸린 시점 분석은 참고 문서로 두지 않고 해당 변경 Technical Design의
Context 섹션으로 남깁니다. 코드베이스 전반을 훑는 시점 평가만
...
- [2026-09-kappa-collector-1-1-deep-review-guide.html](2026-09-kappa-collector-1-1-deep-review-guide.html):
  코드 가이드 2부. 바로 위 변경을 딥 리뷰가 다시 훑어 찾은 결함들과 수정을 다룹니다. 결함들은
  하나같이 "정답에 가까운 다른 곳"에서 값을 읽고 있었습니다. 행 목록이 비어 있으면 진행도를
  0/13이라 단언하던 Kappa 게이지, FIR 절반만 보고 완료라 답해 진행 바와 취소선까지 끌고 가던
