...
### DB가 스스로 밝히는 data format

발행되는 `tarkov_data.db`에는 SQLite의 `PRAGMA user_version`으로 그 시점의 data format이
찍혀 있습니다(SQLite가 애플리케이션용으로 예약해 두고 자기는 읽지 않는 32비트 슬롯입니다).
매니페스트도 data format을 말하지만 그건 **발행자가 페이로드를 설명하는 것**이고, 스탬프는
**페이로드가 자기를 설명하는 것**입니다. 그래서 매니페스트끼리는 앞뒤가 맞으면서도 엉뚱한
